...
// ─── Session key (unique per browser tab, persists on refresh) ────────────────
...
// Converts **text** → <strong>text</strong> and • → bullet in JSX
...
export default function ChatbotWidget({
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      // ✅ CORRECT payload — matches Django chat/views.py ChatView
      const res = await fetch(apiEndpoint, {
        method : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body   : JSON.stringify({
          session_key: sessionKey,   // unique browser session id
          message    : text,         // just the latest message text
        }),
      });
