  ...
  useEffect(() => {
    const loadBlog = async () => {
      try {
        ...
        // silently ignore — submenu stays with "All Posts" only
      }
    };
    loadBlog();
  }, []);
      ...
      try {
        try {
          ...
          if (res.ok) {
            ...
            if (hasData) {
              setNavItems({
                trading:      (page.trading      ?? []).map((i: PortfolioItem) => ({ name: i.name, href: buildPortfolioHref(normalisePortfolioItem(i)) })),
                distribution: (page.distribution ?? []).map((i: PortfolioItem) => ({ name: i.name, href: buildPortfolioHref(normalisePortfolioItem(i)) })),
                services:     (page.services     ?? []).map((i: PortfolioItem) => ({ name: i.name, href: buildPortfolioHref(normalisePortfolioItem(i)) })),
              });
              return;
            }
          }
        ...
        for (const item of active) {
          const normalised = normalisePortfolioItem(item);
          const href = buildPortfolioHref(normalised);
          const entry = { name: normalised.name, href };
