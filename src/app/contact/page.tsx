...
const INITIAL_FORM: EnquiryForm = {
  ...
  subject: '', message: '', enquiry_type: 'general', accepted: false,
}
...
const ContactPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.accepted)       { setErrorMsg('Please accept the Terms & Conditions.'); return }

    setStatus('loading')

    // ── Payload — exactly matches what Postman sends ───────────────────────
    const payload = {
      name:         form.name.trim(),
      email:        form.email.trim(),
      phone:        form.phone.trim(),
      message:      form.message.trim(),
      enquiry_type: form.enquiry_type || 'general',
      subject:      form.subject.trim() || `Enquiry from ${form.name.trim()}`,
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .form-inner select:focus { border-color: #0057C8; }
                    ...
                    <div className="row g-4">
                      ...
                      <div className="col-md-6">
                        <div className="form-inner">
                          ...
                          <select name="enquiry_type" value={form.enquiry_type} onChange={handleChange}>
